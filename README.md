...
3. Navigate to the **Back_End** directory
    ```
...
6. Access in a web browser, Postman, or other application
...
3. Reset TTL on every GET Request.
    - The `get_keys()` function of the `KeyValStore` class will handle the update and removal of keys based on TTL.
4. Has to be FAST
...
6. Must be fault-tolerant, persistent
    ...
    - The `KeyValStore` class used for persistent data
...
**Search** -> search is implemented on the _name_ and *name_en* fields of the Dealer model. Partial matches included, leave blank for all.
...
1. Cursor-based pagination List API in Django
    - I used a Django backend with its REST framework which includes cursor pagination.
2. Dummy data
    - all data is in the Django default SQLite database.
    - 100 dealer table entries created with values generated by the Faker library.
...
7. Application can be executed using Docker
    - Both the backend and frontend are created with separate Docker images, and the containers can be set up and run individually.
...
8. A few things I would improve:
    - In the real world, there would be a security layer for accessing the API. This would most likely be a token authorization system. However, I did not feel the need to implement it for this exercise.
    ...
    - I would implement a proper database for a production environment, with MySQL or similar, and create a more robust database model for the various application entities.
